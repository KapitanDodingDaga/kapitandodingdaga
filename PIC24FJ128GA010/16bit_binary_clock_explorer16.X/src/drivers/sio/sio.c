...
{
//    io_printf("\n\r sio_cmd_buffer.command = %s ", sio_cmd_buffer.command );
...
static void sio_command_string_detected(void)
{
    ...
    // TODO: inform system statemachine that a command is detected
    // note that command detection is triggered by "carriage return or 0xD" character
    if( HUVENT_RESP_OK != huvent_que_post( HUVENT_COMMAND ) )
    {
        while(1);
    }
...
void sio_rx_isr(void)
{
    ...
    // command string is triggered by CARRIAGE_RETURN characted
    if( CARRIAGE_RETURN == sio_rx_buffer.payload[sio_rx_buffer.payload_count] )
    {
        sio_command_string_detected();
        sio_rx_buffer.payload_count = 0;
    }
    else
    {
